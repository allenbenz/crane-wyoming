...
//! `synthesize-stopped` (streamed text synthesis), `audio-start`,
//! `audio-chunk`, `audio-stop`,
//! `voice-started`, `voice-stopped`, `transcribe`, `transcript`,
//! `transcript-start`, `transcript-chunk`, `transcript-stop`, `describe`/
//! `info`, `ping`/`pong`, and `error`. Wake-word and intent events are not
//! included. Unrecognized event types are preserved as [`Event::Unknown`]
